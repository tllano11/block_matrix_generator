...
$(TARGET): $(OUT_PATH)/$(SRC_GEN).o $(OUT_PATH)/$(SRC_SOLVER).o $(OUT_PATH)/$(SRC_JACOBI).o
...
$(OUT_PATH)/$(SRC_SOLVER).o: SLAE_Solver/$(SRC_SOLVER).cpp
	$(CXX) $(CXXFLAGS) $(LDFLAGS) $(CUDAFLAGS) -c $^ -o $@

$(OUT_PATH)/$(SRC_GPU_MUL).o: $(SRC_GPU_MUL).cu
	$(NVCC) $(NVCCFLAGS) -c $^ -o $@

$(OUT_PATH)/$(SRC_JACOBI).o: SLAE_Solver/$(SRC_GPU_JACOBI).cu
	$(NVCC) $(NVCCFLAGS) -c $^ -o $@
